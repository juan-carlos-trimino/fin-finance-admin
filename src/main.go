...
$ go build -o finance-admin && HTTPS=true ./finance-admin

To change the PORT.
$ go build -o finance-admin && HTTPS=true HTTPS_PORT=8181 ./finance-admin

Compile and run the app in the background.
$ go build -o finance-admin && HTTPS=true ./finance-admin &
$ go build -o finance-admin && HTTP=true HTTPS=true LE_CERT=false ./finance-admin

Force rebuilding of packages.
$ go build -o finance-admin -a && HTTPS=true ./finance-admin
...
func makeHandlers() *handlers {
  ...
  var wfadmin = webfinances.WfAdminPages{}
  /***
  ...
  ***/
  ...
  // h.mux["/admin"] = middlewares.ValidateSessions(wfpages.AdminPage)

  h.mux["/admin"] = middlewares.ValidateSessions(wfadmin.AdminPages)
...
func SetupDirStructure(dir string) {
  filePath := dir + "/" + "admin.txt"
  ok, err := osu.CheckDirExists(dir)
  if err != nil {
    panic("Cannot check if directory exists: " + err.Error())
  } else if !ok {
    _, err := osu.CreateDirs(0o077, 0o777, dir)
    if err != nil {
      panic("Cannot create directory " + dir + ": " + err.Error())
    }
    sessions.AddUserToFile(filePath, "admin", "admin")
  }
  //
  if err := sessions.ReadUsersFromFile(filePath); err != nil {
    panic("Cannot read admin file: " + err.Error())
  }
}
